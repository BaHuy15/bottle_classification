...
# python segment/train.py --data coco.yaml --cfg models/segment/yolov7-seg.yaml --name test_KLA_format  --weights yolov5s-seg.pt --img 640 --device 0,1
...
from utils.convert_kla_format_multiclass import convert_format_train_data
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg, resume, noval, nosave, workers, freeze, save_period, mask_ratio = \
        Path(opt.save_dir), opt.epochs, opt.batch_size, opt.weights, opt.single_cls, opt.evolve, opt.data, opt.cfg, \
        opt.resume, opt.noval, opt.nosave, opt.workers, opt.freeze, opt.save_period , opt.mask_ratio
    ...
    try:
        ...
        if str(data).endswith('.yaml'):
            assert os.path.exists(data)
            with open(data, mode='r') as stream:
                data_info = yaml.safe_load(stream)
            
            source_path = os.path.abspath(data_info['path'])
            if ('train' in os.listdir(source_path)) and ('images' not in os.listdir(source_path)):
                check_coco_format = False
                dst_path = os.path.join(source_path, 'train_data')
                if not os.path.exists(dst_path):
                    os.mkdir(dst_path)
                convert_format_train_data(source=source_path, dst=dst_path, class_names=data_info['names'])
                
                with open(data, mode='w') as stream:
                    data_info['path'] = dst_path
                    yaml.dump(data=data_info, stream=stream, sort_keys=False)
        ...
        for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
            ...
            if RANK in {-1, 0}:
                ...
                if (not nosave) or (final_epoch and not evolve):  # if save
                    ...
                    if best_fitness == fi:
                        best = w / f'epoch_{epoch}_best.pt'
